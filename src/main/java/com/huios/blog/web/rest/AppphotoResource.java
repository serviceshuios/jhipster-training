...
import com.huios.blog.service.AppphotoQueryService;
import com.huios.blog.service.AppphotoService;
import com.huios.blog.service.criteria.AppphotoCriteria;
...
/**
 * REST controller for managing {@link com.huios.blog.domain.Appphoto}.
 */
@RestController
@RequestMapping("/api")
public class AppphotoResource {
    ...
    /**
     * {@code SEARCH  /_search/appphotos?query=:query} : search for the appphoto corresponding
     * to the query.
     *
     * @param query the query of the appphoto search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/appphotos")
    public ResponseEntity<List<AppphotoDTO>> searchAppphotos(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Appphotos for query {}", query);
        Page<AppphotoDTO> page = appphotoService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
