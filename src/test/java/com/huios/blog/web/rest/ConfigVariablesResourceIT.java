...
import com.huios.blog.repository.ConfigVariablesRepository;
import com.huios.blog.repository.search.ConfigVariablesSearchRepository;
import com.huios.blog.service.criteria.ConfigVariablesCriteria;
import com.huios.blog.service.dto.ConfigVariablesDTO;
import com.huios.blog.service.mapper.ConfigVariablesMapper;
...
class ConfigVariablesResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/config-variables";
     ...
     *
     * @see com.huios.blog.repository.search.ConfigVariablesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createConfigVariables() throws Exception {
        ...
        // Validate the ConfigVariables in Elasticsearch
        verify(mockConfigVariablesSearchRepository, times(1)).save(testConfigVariables);
    }
    ...
    void createConfigVariablesWithExistingId() throws Exception {
        ...
        // Validate the ConfigVariables in Elasticsearch
        verify(mockConfigVariablesSearchRepository, times(0)).save(configVariables);
    }
    ...
    void putNewConfigVariables() throws Exception {
        ...
        // Validate the ConfigVariables in Elasticsearch
        verify(mockConfigVariablesSearchRepository).save(testConfigVariables);
    }
    ...
    void deleteConfigVariables() throws Exception {
        ...
        // Validate the ConfigVariables in Elasticsearch
        verify(mockConfigVariablesSearchRepository, times(1)).deleteById(configVariables.getId());
    }

    @Test
    @Transactional
    void searchConfigVariables() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        configVariablesRepository.saveAndFlush(configVariables);
        when(mockConfigVariablesSearchRepository.search(queryStringQuery("id:" + configVariables.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(configVariables), PageRequest.of(0, 1), 1));

        // Search the configVariables
        restConfigVariablesMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + configVariables.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(configVariables.getId().intValue())))
            .andExpect(jsonPath("$.[*].configVarLong1").value(hasItem(DEFAULT_CONFIG_VAR_LONG_1.intValue())))
