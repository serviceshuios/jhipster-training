...
import com.huios.blog.service.criteria.FeedbackCriteria;
...
class FeedbackResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/feedbacks";
     ...
     *
     * @see com.huios.blog.repository.search.FeedbackSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void putNewFeedback() throws Exception {
        ...
        // Validate the Feedback in Elasticsearch
        verify(mockFeedbackSearchRepository).save(testFeedback);
    }
    ...
    void searchFeedback() throws Exception {
        ...
        // Search the feedback
        restFeedbackMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + feedback.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(feedback.getId().intValue())))
            .andExpect(jsonPath("$.[*].creationDate").value(hasItem(DEFAULT_CREATION_DATE.toString())))
